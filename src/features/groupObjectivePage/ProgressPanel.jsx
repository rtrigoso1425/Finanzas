...
const CircularProgress = ({ value, label, sublabel, color = "#10b981", sublabelColor = "text-slate-500" }) => {
    const radius = 60; // Radio más grande
...
const ProgressPanel = ({ groupObjective }) => {

    const remainingAmount = Number(groupObjective.remaining_amount ?? 0);
    const totalAmount = Number(groupObjective.total_amount ?? 0);
    ...
    const daysRemaining = useMemo(() => {
        if (!groupObjective?.end_date) return 0;
        const end = new Date(groupObjective.end_date);
        const now = new Date();
    ...
    const progress = useMemo(() => {
        const total = Number(groupObjective?.total_amount ?? 0);
        const remaining = Number(groupObjective?.remaining_amount ?? 0);
        if (!total) return 0;
        ...
        <Card className="border-slate-100 shadow-sm overflow-hidden py-4 sm:py-8">
            <CardContent>
                <div className="flex flex-col sm:flex-row items-center justify-around gap-12 sm:gap-8">
                    {/* Círculo 1: Tiempo Restante */}
                    <div className="flex flex-col items-center gap-4">
                        <CircularProgress 
                            value={(daysRemaining / 30) * 100} // Simulando mes para el visual
                            label={`${daysRemaining}`} 
                            sublabel="Días Restantes" 
                            color="#10b981"
                        />
                    {groupObjective.end_date && (
                        <Badge variant="outline" className="text-xs text-slate-500 border-slate-200">
                            Cierre: {new Date(groupObjective.end_date).toLocaleDateString()}
                        </Badge>
                    )}
                </div>
                ...
                <div className="hidden sm:block w-px h-32 bg-slate-100"></div>
                    ...
                    <div className="flex flex-col items-center gap-4">
                        <CircularProgress 
                            ...
                            color="#10b981"
                        />
                        <Badge variant="secondary" className="text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-100">
