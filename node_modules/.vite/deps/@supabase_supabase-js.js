...
function getClientPlatformVersion() {
  const _process = globalThis["process"];
  if (_process) {
    const processVersions = _process["versions"];
    if (processVersions && processVersions["node"]) {
      if (typeof window === "undefined") try {
        ...
        if (_require) return _require("os").release();
      } catch (error) {
        return null;
      }
    }
  }
  if (typeof Deno !== "undefined" && Deno.osRelease) try {
    return Deno.osRelease();
  } catch (error) {
    return null;
  }
  if (typeof navigator !== "undefined") {
    if (navigator.userAgentData && navigator.userAgentData.platformVersion) return navigator.userAgentData.platformVersion;
  }
  return null;
}
function getClientRuntime() {
  ...
  if (_process) {
    const processVersions = _process["versions"];
    if (processVersions && processVersions["node"]) return "node";
  }
  return null;
}
function getClientRuntimeVersion() {
  if (typeof Deno !== "undefined" && Deno.version) return Deno.version.deno;
  if (typeof Bun !== "undefined" && Bun.version) return Bun.version;
  const _process = globalThis["process"];
  if (_process) {
    const processVersions = _process["versions"];
    if (processVersions && processVersions["node"]) return processVersions["node"];
  }
  return null;
}
function buildHeaders() {
  const headers = { "X-Client-Info": `supabase-js-${JS_ENV}/${version4}` };
  const platform = getClientPlatform();
  if (platform) headers["X-Supabase-Client-Platform"] = platform;
  const platformVersion = getClientPlatformVersion();
  if (platformVersion) headers["X-Supabase-Client-Platform-Version"] = platformVersion;
  const runtime = getClientRuntime();
  if (runtime) headers["X-Supabase-Client-Runtime"] = runtime;
  const runtimeVersion = getClientRuntimeVersion();
  if (runtimeVersion) headers["X-Supabase-Client-Runtime-Version"] = runtimeVersion;
  return headers;
}
var DEFAULT_HEADERS3 = buildHeaders();
